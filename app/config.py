...
def _keep_alive_idle_timeout() -> int:
    ...
    next follow-up. 0 parks indefinitely — until the session is stopped or its
    display slot is claimed by a new session.
    """
    ...
    if seconds < 0:
        raise ValueError(
            f"KEEP_ALIVE_IDLE_TIMEOUT must not be negative; got {raw!r}."
        )
    return seconds
...
class Settings:
    ...
    keep_alive_idle_timeout: int = field(
        default_factory=lambda: _keep_alive_idle_timeout()
    )
