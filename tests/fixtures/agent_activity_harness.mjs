...
check("no copy button is rendered at all", !container.innerHTML.includes("ob-activity-copy"), container.innerHTML);
...
check("a finished thought keeps its disclosure", container.innerHTML.includes("ob-activity-chev"), container.innerHTML);
...
check("a phase with no prose claims no prose", !cls("has-prose"), [...container._classes].join(","));
