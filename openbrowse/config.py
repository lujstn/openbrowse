...
def _resolve_home() -> Path:
    """Directory holding ``.env`` and ``data/``.

    ``OPENBROWSE_HOME`` wins; a git checkout keeps everything repo-local as
    before; a wheel install gets ``~/.openbrowse`` so nothing lands in
    site-packages.
    """
...
def _update_check_hours() -> float:
    raw = os.environ.get("UPDATE_CHECK_HOURS", "").strip()
    if not raw:
        return 6.0
    try:
        hours = float(raw)
        ...
        raise ValueError(
            f"UPDATE_CHECK_HOURS must be a number of hours (0 disables); got {raw!r}."
        ) from None
...
class Settings:
    ...
    home_dir: Path = field(default_factory=lambda: _BASE)
    env_path: Path = field(default_factory=lambda: ENV_PATH)
    update_check_hours: float = field(default_factory=_update_check_hours)
